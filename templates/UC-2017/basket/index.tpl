...
<script type="text/javascript">
    dataLayer.push({
        'event': 'rem',
        'ecomm_itemid': [{foreach name=positionen from=$smarty.session.Warenkorb->PositionenArr item=oPosition}'{$oPosition->kArtikel}'{if not $smarty.foreach.positionen.last},{/if}{/foreach}],
        'ecomm_pagetype': 'cart',
        'ecomm_totalvalue': [{foreach name=positionen from=$smarty.session.Warenkorb->PositionenArr item=oPosition}'{$oPosition->Artikel->Preise->fVKBrutto}'{if not $smarty.foreach.positionen.last},{/if}{/foreach}]
    });
</script>

{foreach name=positionen from=$smarty.session.Warenkorb->PositionenArr item=oPosition}{getBasket id=$oPosition->kArtikel price=$oPosition->Artikel->Preise->fVKBrutto}{/foreach}
                            ...
                            {block name="basket-freegift-body"}
                                <form method="post" name="freegift" action="warenkorb.php">
                                    ...
                                    <div class="text-center">
                                        ...
                                        <input name="gratishinzufuegen" type="submit" value="{lang key="addToCart" section="global"}" class="submit btn-d" />
                                    </div>
                                </form>
                            {/block}
                        </div>
